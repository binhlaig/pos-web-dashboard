...
//     // Remote API က status 400/409 ထုတ်တာလည်း 그대로 return
...
//   // avatar file -> ဒီမှာ direct storage မလုပ်ထားဘူး (လိုရင် /uploads သို့ save logic ထပ်ထည့်ပေးမယ်)
...
//   const user = await User.create({ username, passwordHash, role, isActive: true });
...
//   // Remote responded but error (400/409...) => pass through (fallback မလုပ်ချင်ရင် ဒီလို)
//   if (remote && !remote.ok) {
//     // ❗ Remote က "duplicate username" တို့ဆို fallback မလုပ်သင့်
//     return NextResponse.json(remote.data, { status: remote.status });
//   }

//   // 2) network fail => fallback to local mongo
