...
/* ===== EntriesTable ===== */
function EntriesTable({ shifts, now, getEmployee, staffMap, onSelectStaff, onEdit, onDelete }: {
  shifts: Shift[]; now: number;
  getEmployee: (id: string) => { id: string; name: string; dept?: string } | null;
  staffMap: Record<string, Staff>;
  onSelectStaff: (employeePkId: string) => void;
  onEdit: (s: Shift) => void;
        ...
        {sorted.map((s) => {
          ...
          return (
            <div
              ...
              role="button"
              tabIndex={0}
              onClick={() => onSelectStaff(s.employeeId)}
              onKeyDown={(event) => {
                if (event.key === "Enter" || event.key === " ") {
                  event.preventDefault();
                  onSelectStaff(s.employeeId);
                }
              }}
              className="cursor-pointer rounded-2xl border border-[rgba(200,137,42,0.12)] bg-white p-4 shadow-sm transition hover:border-amber-300 hover:bg-amber-50/50 focus:outline-none focus:ring-2 focus:ring-amber-400/50 dark:border-[rgba(255,255,255,0.08)] dark:bg-[rgba(255,255,255,0.03)] dark:hover:border-amber-400/30 dark:hover:bg-amber-500/[0.06]"
            >
              ...
              <div className="mt-4 flex items-center gap-2">
                <Button variant="outline" size="icon" className="h-9 w-9" onClick={(event) => { event.stopPropagation(); onEdit(s); }}><Pencil className="h-4 w-4" /></Button>
                <Button variant="outline" size="icon" className="h-9 w-9" onClick={(event) => { event.stopPropagation(); onDelete(s); }}><Trash className="h-4 w-4" /></Button>
              </div>
            </div>
          );
        })}
              ...
              {sorted.map((s) => {
                ...
                return (
                  <tr
                    ...
                    className={cn(
                      "cursor-pointer transition hover:bg-amber-50/70 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-amber-400/50 dark:hover:bg-amber-500/[0.06]",
                      isLive && "bg-amber-50/70 dark:bg-[rgba(200,137,42,0.08)]"
                    )}
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <Button variant="outline" size="icon" className="h-8 w-8" onClick={(event) => { event.stopPropagation(); onEdit(s); }}><Pencil className="h-4 w-4" /></Button>
                        <Button variant="outline" size="icon" className="h-8 w-8" onClick={(event) => { event.stopPropagation(); onDelete(s); }}><Trash className="h-4 w-4" /></Button>
                      </div>
                    </td>
                  </tr>
                );
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-5">
          {loading ? (
            ...
            <>
                ...
                {pagedStaff.map((s) => {
                  ...
                  return (
                    <div key={String(s.id)} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-white/5">
                      ...
                      <div>
                        <Button size="sm" className="w-full rounded-xl bg-[#a07020] text-white hover:bg-[#8a6118] dark:bg-[#a07020]" onClick={() => onOpenTimecard(s)}>
              ...
              {/* Desktop table */}
              <div className="hidden overflow-x-auto rounded-2xl border border-slate-200 sm:block dark:border-white/10">
                <table className="w-full min-w-[920px] table-fixed text-sm">
                  <thead className="sticky top-0 z-10 bg-[#faf6f0] dark:bg-[#18120d]">
                    <tr className="border-b border-slate-200 dark:border-white/10 text-xs uppercase tracking-wide text-[#8a6c47] dark:text-[#8a7a65]">
                      <th className="w-16 px-3 py-3 text-left">ID</th>
                      <th className="w-24 px-3 py-3 text-left">Staff ID</th>
                      <th className="w-[220px] px-3 py-3 text-left">Name / Email</th>
                      <th className="w-28 px-3 py-3 text-left">Role</th>
                      <th className="w-32 px-3 py-3 text-left">Branch</th>
                      <th className="w-28 px-3 py-3 text-left">Shop</th>
                      <th className="w-28 px-3 py-3 text-left">Status</th>
                      <th className="w-36 px-3 py-3 text-center">Action</th>
                    </tr>
                  </thead>
                    ...
                    {pagedStaff.map((s, i) => {
                      ...
                      return (
                        <tr key={String(s.id)} className={cn(
                          ...
                          <td className="px-3 py-3 font-mono text-slate-700 dark:text-[#d4c3ab]">{String(s.id)}</td>
                          <td className="px-3 py-3 font-mono text-slate-700 dark:text-[#d4c3ab]">{sid}</td>
                          <td className="px-3 py-3">
                            <div className="max-w-[200px] truncate font-medium text-slate-900 dark:text-[#f3e7d2]">{name}</div>
                            <div className="max-w-[200px] truncate text-xs text-slate-500 dark:text-[#9f8d78]">{String(s.email ?? "—")}</div>
                          </td>
                          <td className="px-3 py-3 text-slate-700 dark:text-[#e5d4bc]"><div className="truncate">{String(s.role ?? "—")}</div></td>
                          <td className="px-3 py-3 text-slate-700 dark:text-[#e5d4bc]"><div className="truncate">{String(s.branch ?? "—")}</div></td>
                          <td className="px-3 py-3 text-slate-700 dark:text-[#e5d4bc]"><div className="truncate">{shop}</div></td>
                          <td className="px-3 py-3">
                            <Badge className={cn("rounded-full border text-xs", statusCls(String(s.status ?? "")))}>{String(s.status ?? "—")}</Badge>
                          </td>
                          <td className="px-3 py-3">
                            <div className="flex justify-center">
                              <Button size="sm" className="h-8 w-full whitespace-nowrap rounded-lg bg-[#a07020] px-3 text-xs text-white hover:bg-[#8a6118] dark:bg-[#a07020]" onClick={() => onOpenTimecard(s)}>
...
export default function TimecardPro() {
  ...
  const [todayShopShifts, setTodayShopShifts] = React.useState<Shift[]>([]);
  const [loadingTodayShifts, setLoadingTodayShifts] = React.useState(false);
  const showAll = false;
  ...
  const [staffDialog, setStaffDialog] = React.useState<Staff | null>(null);
  const [staffDialogAction, setStaffDialogAction] = React.useState<
  ...
  const fetchAllStaff = React.useCallback(async () => {
    ...
    try {
      ...
      const path = query
        ? `/api/timecard/staff?${query}`
        : "/api/timecard/staff";
      const res = await get<Staff[]>(path, accessToken);
  ...
  const fetchShiftsForId = React.useCallback(async (pkId: string | null, all: boolean, sid?: string) => {
    try {
      ...
      if (all) {
        ...
        path = qs ? `/api/timecard/shifts?${qs}` : "/api/timecard/shifts";
      } else if (pkId) {
        const params = new URLSearchParams();
        params.set("employeeId", String(Number(pkId)));
        if (resolvedShopId) params.set("shopId", resolvedShopId);
        path = `/api/timecard/shifts?${params.toString()}`;
      } else {
        path = null;
      }
  ...
  const fetchTodayShopShifts = React.useCallback(async () => {
    ...
    try {
      setLoadingTodayShifts(true);
      const params = new URLSearchParams();
      params.set("shopId", shopId);
      const res = await get<ApiShift[]>(
        `/api/timecard/shifts?${params.toString()}`,
        accessToken
      );
      const today = toYmd(new Date());
      const mapped = (Array.isArray(res) ? res : []).map(mapApiShift);
      setTodayShopShifts(
        mapped.filter(
          (shift) =>
            shift.date === today &&
            (!shift.shopId || String(shift.shopId) === shopId)
        )
      );
    } catch (e: any) {
      toast.error(e?.message || "Failed to load today's clock-ins");
      setTodayShopShifts([]);
    } finally {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const s = await get<Staff>(`/api/timecard/staff/by-staff-id/${employeeId}`, accessToken);
        if (cancelled) return;
  ...
  const selectStaffFromEntry = React.useCallback(
    (employeePkId: string) => {
      const selected =
        staffMap[String(employeePkId)] ??
        staffCacheRef.current[String(employeePkId)];

      if (!selected) {
        toast.error("Staff data is still loading. Please try again.");
        void fetchAllStaff();
        return;
      }

      const realStaffId = selected.staffId ?? selected.staff_id;
      if (realStaffId == null || String(realStaffId).trim() === "") {
        toast.error("This employee does not have a Staff ID.");
        return;
      }

      const pkId = String(selected.id);
      setEmployeeId(String(realStaffId));
      setStaffPkId(pkId);
      setStaff(selected);
      setSelectedStaffName(staffDisplayName(selected));
      staffCacheRef.current[pkId] = selected;
      void fetchShiftsForId(pkId, false, shopId);
  ...
  const staffDialogShifts = React.useMemo(
    () =>
      shifts
        .filter((s) => s.employeeId === staffDialogPkId)
        .slice()
  ...
  const allFiltered = React.useMemo(() => {
    return shifts.filter((s) => {
      ...
      const shopOk = showAll
        ? !shopId || currentShopStaffIds.size === 0 || currentShopStaffIds.has(String(s.employeeId))
        : true;
  ...
  const doClockIn = async () => {
    ...
    try {
      ...
      await Promise.all([
        fetchShiftsForId(selectedPkId, false, shopId),
        fetchTodayShopShifts(),
      ]);
    } catch (e: any) {
  ...
  const doClockOut = async () => {
    ...
    try {
      await post<ApiShift>(
        `/api/timecard/${active.id}/clock-out`,
        undefined,
        accessToken
      );
  ...
  const startBreak = async () => {
    ...
    try {
      await post<ApiShift>(
        `/api/timecard/${active.id}/break/start`,
        undefined,
        accessToken
      );
  ...
  const endBreak = async () => {
    ...
    try {
      await post<ApiShift>(
        `/api/timecard/${active.id}/break/end`,
        undefined,
        accessToken
      );
  ...
  const runStaffDialogAction = async (
    action: "clock-in" | "clock-out" | "break-start" | "break-end"
  ) => {
    if (!staffDialog || staffDialogAction) return;
    ...
    const employeePkId = Number(staffDialog.id);
    if (!Number.isFinite(employeePkId) || employeePkId <= 0) {
      toast.error("Invalid employee");
      return;
    }

    if (action === "clock-in" && staffDialogActiveShift) {
      toast.error("This employee is already clocked in");
      return;
    }
    if (action !== "clock-in" && !staffDialogActiveShift) {
      toast.error("This employee is not clocked in");
      return;
    }
    if (action === "break-start" && staffDialogOnBreak) {
      toast.error("Break is already running");
      return;
    }
    if (action === "break-end" && !staffDialogOnBreak) {
      toast.error("No active break");
      return;
    }

    setStaffDialogAction(action);
    try {
      if (action === "clock-in") {
        await post<ApiShift>(
          "/api/timecard/clock-in",
          {
            employeeId: employeePkId,
            note: note || undefined,
          },
          accessToken
        );
        setNote("");
        toast.success(`${staffDisplayName(staffDialog)} clocked in`);
      } else if (action === "clock-out") {
        await post<ApiShift>(
          `/api/timecard/${staffDialogActiveShift!.id}/clock-out`,
          undefined,
          accessToken
        );
        toast.success(`${staffDisplayName(staffDialog)} clocked out`);
      } else if (action === "break-start") {
        await post<ApiShift>(
          `/api/timecard/${staffDialogActiveShift!.id}/break/start`,
          undefined,
          accessToken
        );
        toast("Break started", { icon: "☕" });
        ...
        await post<ApiShift>(
          `/api/timecard/${staffDialogActiveShift!.id}/break/end`,
          undefined,
          accessToken
        );
        toast.success("Break ended");
      ...
      await Promise.all([
        fetchShiftsForId(null, true, shopId),
        fetchTodayShopShifts(),
      ]);
    } catch (e: any) {
      toast.error(e?.message || "Timecard action failed");
    } finally {
      setStaffDialogAction(null);
    }
  };
  ...
  return (
    <section className="relative min-h-screen overflow-hidden bg-[#f6f1e9] text-slate-900 dark:bg-[#05060d] dark:text-[#f3e7d2]">
                  ...
                  <div className="grid content-start gap-3 rounded-[22px] border border-[rgba(200,137,42,0.14)] bg-[rgba(255,255,255,0.45)] p-5 lg:p-6 dark:bg-[rgba(255,255,255,0.03)]">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          setBrowseOpen(true);
                          void fetchShiftsForId(null, true, shopId);
                        }}
              ...
              <CardContent className="px-6 pb-8 sm:px-8 lg:px-10">
                <Tabs defaultValue="all" className="w-full">
                  ...
                  <TabsContent value="all" className="m-0">
                    {!showAll && !selectedPkId ? (
                      ...
                      <EntriesTable
                        ...
                        onSelectStaff={selectStaffFromEntry}
                  ...
                  <TabsContent value="today" className="m-0">
                    {todays.length === 0 ? (
                      <EmptyState label={loadingTodayShifts ? "Loading today's clock-ins…" : "No clock-in entries for today yet"} />
                    ) : (
                      <EntriesTable
      ...
      {/* Staff detail and timecard actions dialog */}
      <Dialog
        open={!!staffDialog}
        onOpenChange={(open) => {
          if (!open && !staffDialogAction) setStaffDialog(null);
        }}
      >
            ...
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-3">
                  {staffDialog.imageUrl ?? staffDialog.image_url ? (
                    <img
                      src={staffDialog.imageUrl ?? staffDialog.image_url}
                      alt=""
                      className="h-10 w-10 rounded-full border border-slate-200 object-cover dark:border-white/10"
                    />
                  ) : (
                    <span className="grid h-10 w-10 place-items-center rounded-full bg-amber-100 text-amber-800 dark:bg-amber-500/10 dark:text-amber-300">
                      <UserRound className="h-5 w-5" />
                    </span>
                  )}
                  <span className="min-w-0">
                    <span className="block truncate">{staffDisplayName(staffDialog)}</span>
                    <span className="mt-1 block text-xs font-normal text-slate-500 dark:text-white/60">
                      Staff ID: {String(staffDialog.staffId ?? staffDialog.staff_id ?? "—")}
                      {staffDialog.role ? ` · ${staffDialog.role}` : ""}
                    </span>
                  </span>
                </DialogTitle>
              </DialogHeader>
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Badge
                  className={cn(
                    "border",
                    staffDialogActiveShift
                      ? staffDialogOnBreak
                        ? "border-amber-300 bg-amber-100 text-amber-800 dark:border-amber-400/30 dark:bg-amber-500/10 dark:text-amber-200"
                        : "border-emerald-300 bg-emerald-100 text-emerald-800 dark:border-emerald-400/30 dark:bg-emerald-500/10 dark:text-emerald-200"
                      : "border-slate-300 bg-slate-100 text-slate-700 dark:border-white/10 dark:bg-white/10 dark:text-white/70"
                  )}
                >
                  {loadingShifts
                    ? "Loading status…"
              ...
              {staffDialogActiveShift && (
                <div className="grid grid-cols-3 gap-2 rounded-2xl border border-emerald-200 bg-emerald-50/70 p-3 text-center dark:border-emerald-400/20 dark:bg-emerald-500/[0.06]">
                  <div className="min-w-0">
                    <p className="text-xs text-slate-500 dark:text-white/50">Clock In</p>
                    <p className="mt-1 truncate text-sm font-semibold">{prettyClock(staffDialogActiveShift.clockIn)}</p>
                  </div>
                  <div className="min-w-0">
                    <p className="text-xs text-slate-500 dark:text-white/50">Worked</p>
                    <p className="mt-1 truncate text-sm font-semibold">{prettyHM(workMs(staffDialogActiveShift, nowMs))}</p>
                  </div>
                  <div className="min-w-0">
                    <p className="text-xs text-slate-500 dark:text-white/50">Break</p>
                    <p className="mt-1 truncate text-sm font-semibold">{prettyHM(breakMs(staffDialogActiveShift, nowMs))}</p>
                  </div>
                </div>
              )}
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                <Button
                  onClick={() => void runStaffDialogAction("clock-in")}
                  disabled={loadingShifts || !!staffDialogActiveShift || !!staffDialogAction}
                  className="bg-emerald-600 text-white hover:bg-emerald-700"
                >
                  {staffDialogAction === "clock-in" ? (
                ...
                <Button
                  onClick={() => void runStaffDialogAction("clock-out")}
                  disabled={loadingShifts || !staffDialogActiveShift || !!staffDialogAction}
                  variant="destructive"
                >
                  {staffDialogAction === "clock-out" ? (
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                  ) : (
                ...
                <Button
                  onClick={() => void runStaffDialogAction("break-start")}
                  disabled={loadingShifts || !staffDialogActiveShift || staffDialogOnBreak || !!staffDialogAction}
                  variant="outline"
                >
                  {staffDialogAction === "break-start" ? (
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                  ) : (
                ...
                <Button
                  onClick={() => void runStaffDialogAction("break-end")}
                  disabled={loadingShifts || !staffDialogActiveShift || !staffDialogOnBreak || !!staffDialogAction}
                  variant="outline"
                >
                  {staffDialogAction === "break-end" ? (
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                  ) : (
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-semibold text-slate-900 dark:text-[#f3e7d2]">
                    Recent Timecard
                  </h3>
                  <span className="text-xs text-slate-500 dark:text-white/50">
                    Last {staffDialogShifts.length} records
                  </span>
                </div>

                {loadingShifts && staffDialogShifts.length === 0 ? (
                  <div className="flex items-center justify-center gap-2 rounded-xl border border-slate-200 py-5 text-sm text-slate-500 dark:border-white/10 dark:text-white/60">
                    <Loader2 className="h-4 w-4 animate-spin" /> Loading records…
                  </div>
                ) : staffDialogShifts.length === 0 ? (
                  <div className="rounded-xl border border-dashed border-slate-200 py-5 text-center text-sm text-slate-500 dark:border-white/10 dark:text-white/60">
                    No timecard records yet
                  </div>
                ) : (
                  <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-white/10">
                    <table className="w-full min-w-[390px] table-fixed text-xs">
                      <thead className="bg-slate-50 text-slate-500 dark:bg-white/[0.04] dark:text-white/50">
                        <tr>
                          <th className="w-[92px] px-2 py-2 text-left">Date</th>
                          <th className="w-[120px] px-2 py-2 text-left">In → Out</th>
                          <th className="w-[88px] px-2 py-2 text-left">Worked</th>
                          <th className="w-[88px] px-2 py-2 text-left">Break</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100 dark:divide-white/10">
                        {staffDialogShifts.map((shift) => (
      ...
      <AllStaffDialog
        ...
        onOpenTimecard={(s) => {
          ...
          setBrowseOpen(false);
          void fetchShiftsForId(pkId, false, shopId);
          requestAnimationFrame(() => setStaffDialog(s));
        }}
