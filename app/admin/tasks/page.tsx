...
const getSubject = (s: string) =>
  subjectPalette[s] ?? { color: "#64748b", light: "rgba(100,116,139,.14)" };
...
export default function TasksPage() {
  ...
  const fetchTasks = React.useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          await parseError(res, `Failed to load tasks (${res.status})`),
        );
      }
  ...
  const fetchStaff = React.useCallback(
    async (refresh = false) => {
      try {
        ...
        if (!res.ok) {
          throw new Error(
            await parseError(res, `Failed to load staff (${res.status})`),
          );
        }
  ...
  const assignTask = async (taskId: string, staffId: string) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/api/tasks/${taskId}/assign/${staffId}`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${accessToken}`,
      ...
      <header className="mx-auto flex max-w-[1440px] flex-col gap-4 pb-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-[var(--muted)]">
            ...
            <span>{stats.totalStaff} staff connected</span>
          </div>
        </div>
